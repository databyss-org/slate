...
function configure(pkg, env, target) {
  ...
  const input = `packages/${pkg.path}/src/index.ts`
  const deps = []
  ...
  const plugins = [
    ...
    typescript({
      abortOnError: false,
      tsconfig: `./packages/${pkg.path}/tsconfig.json`,
  ...
  if (isUmd) {
    return {
      ...
      output: {
        format: 'umd',
        file: `packages/${pkg.path}/${isProd ? pkg.umdMin : pkg.umd}`,
        exports: 'named',
        name: startCase(pkg.path).replace(/ /g, ''),
  ...
  if (isModule) {
    return {
      ...
      output: [
        {
          file: `packages/${pkg.path}/${pkg.module}`,
          format: 'es',
          sourcemap: true,
        },
        {
          file: `packages/${pkg.path}/${pkg.main}`,
          format: 'cjs',
          exports: 'named',
          sourcemap: true,
        },
